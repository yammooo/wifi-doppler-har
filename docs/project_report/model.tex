...
The teacher follows the reproduced SHARP chain
...
\(\floorval\approx0.0631\). The network targets are the four maps recomputed
with this implementation. Separately, the frozen HAR classifier was trained in
the provided reproduction notebook on \emph{legacy} SHARP maps. Consequently,
the experiments contain two reference distributions: recomputed maps define
pixel fidelity, while legacy maps define the classifier's expected input
distribution. We preserve this distinction rather than treating classifier
accuracy as a pure reconstruction score.

The final coherent experiment uses 164 recordings from all 18 AR scenarios in
the public 80-MHz CSI dataset \cite{meneghello2023dataset}. Within every
recording, the first \(80\%\) is available for training, \(80\)--\(90\%\) for
validation, and \(90\)--\(100\%\) for final testing. All scenarios contribute
to training and source validation; the classifier-compatible scenarios
AR-1a/1b/1c determine early stopping and final reporting. A 31-frame guard
between intervals prevents context leakage. Training windows use stride 170,
while the final frozen-classifier evaluation uses stride 30 for denser
coverage. Uncompressed NumPy memory maps and deterministic recording-aware
windowing make this protocol reproducible without changing its samples.
...
Most target energy lies near zero Doppler: only \(28.4\%\) of scanned
antenna--time frames exceed 0.2 outside the central bins. Plain MSE therefore
rewards a stationary center ridge. We use
\begin{equation}
...
motion-rich. Every epoch includes all 3,646 rich windows and an equal-size,
...
mixed precision, a 50-epoch limit, and patience 8 on AR-1a/1b/1c validation
loss. Training stopped after epoch 25 and restored the best checkpoint from
epoch 17.
